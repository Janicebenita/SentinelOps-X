...
| A2A | Typed, persisted, correlated and trace-propagated handoff tests | managed A2A service required | IMPLEMENTED_AND_VERIFIED |
| MCP | Authenticated read-only tool gateway and mutation tests |  deployed MCP Cloud Run URL | IMPLEMENTED_AND_VERIFIED |
| Managed forecast | Deterministic authoritative fallback and supplemental boundary | Gemini Enterprise Agent Platform invocation | LOCAL_FALLBACK_AVAILABLE |
| BigQuery | Nine physical partitioned/clustered DDL files plus credentialed provision/write/read tooling | dataset row/query job ID | IMPLEMENTED |
| Pub/Sub | Typed idempotent adapter plus provisioning and publish/consume smoke tooling | managed publish/consume message ID | IMPLEMENTED |
| Cloud Run | Images/manifests and CI container builds | `run.app` URL, revision, IAM or health evidence | IMPLEMENTED |
| OpenTelemetry | Local trace IDs propagate through enterprise adapters | Cloud Trace span ID | LOCAL_ADAPTER_ONLY |
...
| Audit and Evidence ZIP | Hash-chain and archive validation tests; live Render probe | Google-managed storage evidence | IMPLEMENTED_AND_VERIFIED |
