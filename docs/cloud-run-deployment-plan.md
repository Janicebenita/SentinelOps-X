...
Build with `gcloud builds submit --config cloudbuild.yaml --substitutions=_REGION=us-central1`. Apply service YAML only after replacing project and image placeholders. Verify every `/health` and `/readiness`, private IAM, frontend API routing, integration calls, audit verification and evidence export. Roll back with `gcloud run services update-traffic SERVICE --to-revisions PREVIOUS=100 --region REGION`.
