...
def execute(db: Session, run: NexusRun, name: str, actor: str, rerun: bool=False) -> AgentWorkspace:
    ...
    delegation=A2AMessage(workflow_id=run.id,correlation_id=f"workflow-{run.id}",sender_agent="api-gateway" if name=="nexus-orchestrator" else "nexus-orchestrator",receiver_agent=name,action="rerun" if rerun else "run",input_artifacts=[row.input_reference],retry_count=retry,expires_at=expiry())
    ...
    completion=A2AMessage(workflow_id=run.id,task_id=delegation.task_id,correlation_id=delegation.correlation_id,sender_agent=name,receiver_agent="nexus-orchestrator",action="result",status="completed" if row.status=="COMPLETED" else "rejected",output_artifacts=[row.output_reference] if row.output_reference else [],evidence_ids=row.evidence_ids,error=row.error,retry_count=retry,trace_id=delegation.trace_id,expires_at=expiry())
