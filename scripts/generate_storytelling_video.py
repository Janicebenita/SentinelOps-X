...
RATE = "-3%"  # natural, measured professional delivery
SCENE_SECONDS = 30
SECTION_TARGETS = (
...
SCENES = (
    Scene("overview", "Meet SentinelOps Nexus", "Welcome to SentinelOps Nexus, the Enterprise Operational Digital Twin. The Payment Service is healthy now, but traffic and Redis memory pressure are rising. Nexus predicts the next operational bottleneck before customers experience it, tests possible interventions in a bounded simulation, and stops at a human decision. Throughout this demonstration, production action is not executed."),
    Scene("forecast", "Yesterday, now and tomorrow", "The simulated run begins with a healthy baseline. Moving from yesterday to now and then forty-five minutes ahead reveals a steady Redis memory trend. Safe capacity crosses ninety per cent at plus thirty minutes. Customer impact is estimated at plus forty-five, whilst the conventional reactive alert remains quiet. Nexus makes this early-warning window visible and reviewable."),
    Scene("evidence", "Evidence before confidence", "The prediction is transparent. Nexus shows the saturation equation, threshold, residual error and assumptions beside content-hashed evidence. Metrics establish the rising memory slope. Configuration establishes current capacity and replicas. Trace-like events place Redis on the Payment Service critical path. Confidence is a heuristic evidence score, not a probability or a claim of certainty."),
    Scene("twin", "Explore the bounded Twin", "Explore Mode changes the future without touching production. In this simulated run, traffic increases to ten times baseline, Redis capacity remains twelve thousand, replicas remain four, and dependency latency rises to eighty milliseconds. Rerun bounded Twin creates a persisted model with a fixed seed and disabled network, then replays the same documented scenarios. No infrastructure command is available here."),
    Scene("scenarios", "Twelve deterministic futures", "The Scenario Lab now runs twelve futures against the same hashed Twin. It covers baseline growth, Redis crash and latency, replica failover, ten-times traffic, million-user stress, reduced capacity, increased replicas, rollback, rate limiting, cache-policy correction and configuration drift. Selecting a scenario reveals its inputs, recovery time, outcome and deterministic result hash for repeatable comparison."),
    Scene("tournament", "Reject the plausible false fix", "The Intervention Tournament compares FAST, SAFE and OPTIMAL. FAST scales application replicas immediately and appears quick, but the bottleneck is Redis. Its mandatory failover gate fails, so FAST is disqualified regardless of score. SAFE remains eligible. OPTIMAL combines Redis capacity, cache-policy correction and gradual scaling, passes every mandatory gate, and becomes the evidence-backed recommendation."),
    Scene("impact", "Business impact, visibly estimated", "Nexus translates operational risk into an estimated business exposure using visible assumptions: request rate, conversion rate, average order value, the risk window, projected failure rate and an SLA penalty. The arithmetic helps a reviewer compare urgency and intervention cost. It is explicitly an operational estimate, not an accounting result, guaranteed loss, or calibrated financial forecast."),
    Scene("approval", "Human authority is preserved", "Nexus stops at the Human Approval Gateway. The recommendation, contradictory evidence and audit trail are ready for an accountable reviewer. This simulated run does not press approve. Even a real approval would only record a named decision and enable evidence export. It would not deploy, scale replicas, change Redis or call a production control plane. Production action is not executed."),
    Scene("conclusion", "Predict, test, decide safely", "SentinelOps Nexus turns late reactive alerting into an earlier, evidence-backed decision. It predicts the emerging bottleneck, tests interventions across deterministic futures, rejects a convincing false fix, quantifies estimated exposure and preserves human control. Every important claim remains traceable to evidence. Nothing was automatically deployed. Production action was not executed. Thank you for watching."),
)

NEXUS_VISUALS = (
    """<div class='hero'><div><span class='badge ok'>WORKFLOW RECOMMENDED</span><h1>Payment Service · Redis saturation</h1><p>Predict tomorrow's bottleneck before customers experience it</p></div><div class='big bad'>45:00<small>TIME TO CUSTOMER IMPACT</small></div></div><div class='kpis'><article><small>OPERATIONAL HEALTH</small><b>70/100</b></article><article><small>SAFE CAPACITY CROSSING</small><b>+30 min</b></article><article><small>SCENARIOS</small><b>12</b></article><article><small>PRODUCTION ACTION</small><b class='ok'>NOT EXECUTED</b></article></div><div class='panel safety'><b>BOUNDED DIGITAL TWIN · HUMAN APPROVAL REQUIRED · DETERMINISTIC EVIDENCE</b></div>""",
    """<h1>Yesterday · Now · Tomorrow</h1><p class='lead'>The forecast leads the reactive customer alert</p><div class='panel'><svg viewBox='0 0 1500 430'><line x1='70' y1='110' x2='1430' y2='110' stroke='#ff607f' stroke-width='3' stroke-dasharray='12 8'/><text x='75' y='92' fill='#ff8095' font-size='23'>90% SAFE-CAPACITY THRESHOLD</text><polyline class='line' points='70,360 350,335 630,290 900,225 1170,110 1430,55'/><circle cx='1170' cy='110' r='13' fill='#ff607f'/><text x='1080' y='150' fill='#fff' font-size='25'>CROSSING +30</text><text x='70' y='410' fill='#80aeb5' font-size='22'>YESTERDAY</text><text x='580' y='410' fill='#80aeb5' font-size='22'>NOW · 72%</text><text x='1080' y='410' fill='#80aeb5' font-size='22'>+30 · 90%</text><text x='1350' y='410' fill='#80aeb5' font-size='22'>+45</text></svg></div><div class='timeline'><b>REACTIVE ALERT</b><span class='badge bad'>STILL QUIET AT CROSSING</span><b>CUSTOMER IMPACT</b><span class='badge bad'>+45 MINUTES</span></div>""",
    """<h1>Transparent forecast and evidence</h1><p class='lead'>Confidence is evidence strength—not probability</p><div class='split'><div class='panel code'><h2>BOUNDED EQUATION</h2><pre>memory_pct(t) = 72 + 0.6 × minutes\nsafe threshold = 90%\ncrossing = (90 - 72) / 0.6 = 30 min\nerror bound = ± 3 minutes</pre></div><div class='panel'><h2>CONTENT-HASHED EVIDENCE</h2><p><span class='badge ok'>METRICS</span> Rising Redis memory slope</p><p><span class='badge ok'>CONFIG</span> 12,000 capacity · 4 replicas</p><p><span class='badge ok'>TRACE</span> Redis on payment critical path</p><p><span class='badge'>ASSUMPTION</span> Capacity remains fixed</p></div></div><div class='hero compact'><b>HEURISTIC EVIDENCE SCORE</b><div class='big ok'>85/100<small>NOT A CALIBRATED PROBABILITY</small></div></div>""",
    """<h1>Explore Mode · simulated input change</h1><p class='lead'>A bounded model—not a production control plane</p><div class='controls panel'><label>TRAFFIC MULTIPLIER<b class='bad'>10×</b></label><label>REDIS CAPACITY<b>12,000</b></label><label>APP REPLICAS<b>4</b></label><label>DEPENDENCY LATENCY<b class='bad'>80 ms</b></label></div><div class='manifest panel'><article><small>TWIN ID</small><b>twin-1aa31b64e280</b></article><article><small>RANDOM SEED</small><b>20260808</b></article><article><small>NETWORK</small><b class='bad'>DISABLED</b></article><article><small>MANIFEST SHA-256</small><b>1aa31b64…e280</b></article><article><small>SOURCE REVISION</small><b>b9a9e4d4f9</b></article><article><small>PRODUCTION MUTATION</small><b class='ok'>NONE</b></article></div><div class='closing pulse'>▶ RERUN BOUNDED TWIN · SIMULATION ONLY</div>""",
    """<h1>Scenario Lab · twelve deterministic futures</h1><p class='lead'>Same Twin · same seed · selectable result evidence</p><div class='matrix panel'><div><b>SCENARIO</b><b>STATUS</b><b>P95</b><b>ERROR</b><b>RECOVERY</b></div><div><span>Baseline Growth</span><i class='x'>FAIL</i><i>620 ms</i><i>7.4%</i><i>45 min</i></div><div><span>Redis Crash</span><i class='x'>FAIL</i><i>5000 ms</i><i>100%</i><i>18 min</i></div><div><span>Replica Failover</span><i class='warn'>DEGRADED</i><i>410 ms</i><i>1.1%</i><i>9 min</i></div><div><span>10× Traffic</span><i class='x'>FAIL</i><i>3400 ms</i><i>42%</i><i>30 min</i></div><div><span>Rate Limiting Intervention</span><i>PASS</i><i>248 ms</i><i>0.4%</i><i>4 min</i></div><div><span>Cache Policy Correction</span><i>PASS</i><i>205 ms</i><i>0.2%</i><i>6 min</i></div></div><div class='hash panel'><small>SELECTED RESULT HASH</small><b>bcb060f5e09fffc7…</b><span class='badge ok'>REPEATABLE</span></div>""",
    """<h1>Intervention Tournament</h1><p class='lead'>Eligibility overrides score · one failed mandatory gate disqualifies</p><div class='three'><article class='panel dangerbox'><span class='badge bad'>FAST · DISQUALIFIED</span><h2>Scale app replicas now</h2><b class='metric'>79.2</b><p>Failover gate <b class='bad'>FAILED</b></p><p>Redis bottleneck remains</p></article><article class='panel'><span class='badge ok'>SAFE · ELIGIBLE</span><h2>Redis capacity + traffic shaping</h2><b class='metric'>84.6</b><p>Mandatory gates <b class='ok'>PASSED</b></p><p>Lower-cost alternative</p></article><article class='panel winner'><span class='badge ok'>OPTIMAL · RECOMMENDED</span><h2>Capacity + cache policy + gradual scale</h2><b class='metric ok'>91.0</b><p>Mandatory gates <b class='ok'>PASSED</b></p><p>Best eligible score</p></article></div><div class='reject'><span>FALSE FIX CAUGHT</span><b>FAST CANNOT OUTSCORE A FAILED SAFETY GATE</b></div>""",
    """<h1>Business impact · visible assumptions</h1><p class='lead'>Operational estimate—not an accounting result</p><div class='split'><div class='panel'><small>ESTIMATED REVENUE EXPOSURE</small><div class='big bad'>₹2.7 crore</div><p>Customers at risk <b>22,000</b></p><p>SLA breach risk <b class='bad'>HIGH</b></p></div><div class='panel code'><h2>VISIBLE EQUATION</h2><pre>requests × conversion rate\n× average order value\n× risk window\n× projected failure rate\n+ SLA penalty</pre></div></div><div class='kpis'><article><small>CONVERSION</small><b>3.4%</b></article><article><small>AVERAGE ORDER</small><b>₹3,200</b></article><article><small>RISK WINDOW</small><b>60 min</b></article><article><small>SLA PENALTY</small><b>₹2.5 lakh</b></article></div>""",
    """<h1>Human Approval Gateway</h1><p class='lead'>The simulated run stops. The agent cannot approve itself.</p><div class='approval panel'><div class='shield'>✓</div><div><span class='badge ok'>AWAITING HUMAN</span><h2>OPTIMAL ready for accountable review</h2><p>Recommendation · contradictory evidence · audit chain · evidence export</p><div class='buttons'><button>REJECT</button><button class='approve'>APPROVE PROPOSAL</button></div></div></div><div class='safety-grid'><b>APPROVAL AUTOMATED <span>NO</span></b><b>PRODUCTION CONTROL CALLED <span>NO</span></b><b>PRODUCTION ACTION <span>NOT EXECUTED</span></b></div>""",
    """<div class='finale-card'><div class='finale-shield'>✓</div><h1>Predict earlier · Test safely · Decide with evidence</h1><p>Emerging bottleneck identified before the reactive alert</p><p>Plausible false fix rejected by mandatory safety gates</p><p>Human authority preserved at the final boundary</p><div class='thank-you'>THANK YOU</div><small>SENTINELOPS NEXUS · ENTERPRISE OPERATIONAL DIGITAL TWIN</small><div class='closing'>PRODUCTION ACTION: NOT EXECUTED</div></div>""",
)
...
async def capture(url: str, durations: list[float]) -> Path:
    ...
    async with async_playwright() as manager:
        ...
        await page.locator(".brand").evaluate("node => node.innerHTML='SENTINEL<span>OPS NEXUS</span>'")
        await page.locator(".mode").evaluate("node => node.textContent='ENTERPRISE OPERATIONAL DIGITAL TWIN · GUIDED SIMULATION'")
        for index, _scene in enumerate(SCENES):
            scene_started = time.monotonic()
            await page.locator("#stage").evaluate("(node, html) => { node.style.animation='none'; node.innerHTML=html; void node.offsetWidth; node.style.animation='enter .65s ease'; }", NEXUS_VISUALS[index])
...
def encode(durations: list[float], recording: Path | None = None) -> tuple[Path, Path]:
    ...
    filters.append(f"{joined}amix=inputs={len(SCENES)}:duration=longest:dropout_transition=0,dynaudnorm=f=250:g=7,loudnorm=I=-16:LRA=5:TP=-1.5,alimiter=limit=0.9,apad=whole_dur={total},afade=t=out:st={fade_start}:d=2[voice]")
