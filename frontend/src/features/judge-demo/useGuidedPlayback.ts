...
export function useGuidedPlayback(length:number,onAdvance:(index:number)=>void,initialIndex=0){
 const [index,setIndex]=useState(initialIndex); const [playing,setPlaying]=useState(false); const [guided,setGuided]=useState(true);
 const select=(next:number,manual=true)=>{const bounded=Math.max(0,Math.min(length-1,next));setIndex(bounded);onAdvance(bounded);if(manual)setPlaying(false)};
