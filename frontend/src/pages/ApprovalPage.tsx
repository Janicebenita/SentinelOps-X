...
export default function ApprovalPage(){
 ...
 const decide=useMutation({mutationFn:(kind:'approve'|'reject'|'request-evidence')=>nexusApi.decide(id,kind,{actor_name:actor,decision:kind==='request-evidence'?'request_more_evidence':kind,rationale,verification_token:verified!.verification_token}),onSuccess:()=>qc.invalidateQueries({queryKey:['workflow',id]})});
 ...
 return <div className="product-page"><nav><a href="/command-centre"><b>SENTINELOPS NEXUS</b></a><a href={`/workflows/${id}/verification`}>Verification</a></nav><main><h1>Human Approval Gateway</h1><p>The Verification Agent validates qualification and readiness. It never approves.</p><section className="approval-form"><label>Actor name<input value={actor} onChange={e=>setActor(e.target.value)} autoComplete="name"/></label><label>Trial access code<input value={code} onChange={e=>setCode(e.target.value)} type="password" autoComplete="one-time-code"/></label><button onClick={()=>verify.mutate()} disabled={actor.length<2||code.length<4||verify.isPending}>{verify.isPending?'Verifying role…':'Verify role'}</button>{verify.isError&&<p className="form-error">{verify.error.message}</p>}{verified&&<div className={`role-badge ${verified.role.toLowerCase()}`}><ShieldCheck/> VERIFIED ROLE: {verified.role.replace('_',' ')}</div>}<label>Mandatory rationale<textarea value={rationale} onChange={e=>setRationale(e.target.value)} rows={4}/></label><div className="decision-actions"><button disabled={!canApprove||decide.isPending} onClick={()=>decide.mutate('approve')}>Approve Recommendation</button><button disabled={!verified||!awaiting||rationale.trim().length<3||decide.isPending} onClick={()=>decide.mutate('reject')}>Reject Recommendation</button><button disabled={!verified||!awaiting||rationale.trim().length<3||decide.isPending} onClick={()=>decide.mutate('request-evidence')}>Request More Evidence</button></div>{verified?.role==='INTERN'&&<p className="form-warning">Intern users may inspect, reject or request evidence, but cannot approve.</p>}{decide.isSuccess&&<p className="form-success">Human decision recorded and audited. No production action occurred.</p>}{decide.isError&&<p className="form-error">{decide.error.message}</p>}</section><div className="boundary"><ShieldCheck/> PRODUCTION ACTION: NOT EXECUTED</div></main></div>;
}
