...
export default function JudgeDemoPage(){
 ...
 const bootstrap=useMutation({mutationFn:nexusApi.bootstrap,onSuccess:async executed=>{cache.setQueryData<NexusRun[]>(['judge-workflows'],rows=>[executed,...(rows??[]).filter(item=>item.id!==executed.id)]);setSelectedRunId(executed.id);await refresh();playback.restart()}});
