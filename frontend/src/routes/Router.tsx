...
export default function Router(){const [path,setPath]=useState(location.pathname);useEffect(()=>{const change=()=>setPath(location.pathname);addEventListener('popstate',change);return()=>removeEventListener('popstate',change)},[]);let Page=Landing;if(path==='/command-centre')Page=CommandCentre;else if(path==='/agents'||path.startsWith('/agents/'))Page=AgentPage;else if(/\/workflows\/\d+\/approval/.test(path))Page=ApprovalPage;else if(path!=='/')Page=ResourcePage;return <Suspense fallback={<div className="route-loading">Loading SentinelOps Nexus…<b>PRODUCTION ACTION: NOT EXECUTED</b></div>}><Page/></Suspense>}
